...
export async function importCarrefourAuthState(
	...
	const targetPath = destinationPath?.trim() ?? config.statePath;
	await mkdir(path.dirname(targetPath), { recursive: true });
	const updatedAt = new Date().toISOString();
	await writeFile(targetPath, JSON.stringify(storageState), {
		encoding: "utf-8",
		mode: 0o600,
	});
...
export async function getCarrefourAuthStatus(): Promise<AuthStatusResult> {
	...
	try {
		const state = JSON.parse(
			await readFile(config.statePath, { encoding: "utf-8" }),
