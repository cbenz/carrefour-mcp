...
import { addToCarrefourCart } from "./tools/addToCart.js";
...
export function createServer(): McpServer {
	...
	server.registerTool(
		"add_to_cart",
		{
			title: "Add products to Carrefour cart",
			description:
				"Add a list of products to the Carrefour cart of the authenticated account using browser automation.",
			inputSchema: {
				items: z
					.array(
						z.object({
							name: z.string().describe("Product name"),
							productId: z.string().optional().describe("Product EAN/ID"),
							productUrl: z
								.string()
								.url()
								.describe("Carrefour product page URL"),
							quantity: z
								.number()
								.int()
								.min(1)
					...
					.min(1)
					.describe("List of products to add to cart"),
			},
		},
		async ({ items }) => {
			try {
				const result = await addToCarrefourCart(items);
				return {
					content: [
						{
							type: "text",
				...
				return {
					content: [
						{
							type: "text",
							text: `add_to_cart failed: ${message}`,
						},
					],
					isError: true,
				};
